const Footer = ({setIndex,index,setUserData,idxNum}) => {
 ...
 <footer className=' lg:absolute lg:bottom-0 text-center w-full align-center justify-center gap-3 flex p-2 bg-black ' >
      <button onClick={()=>{setIndex(index-1),setUserData([]) }}  className='bg-amber-500  p-1 rounded cursor-pointer' >Prev</button>
      ...
      <button  onClick={()=>{setIndex(index+1),setUserData([])  }} className='bg-amber-500  p-1 rounded cursor-pointer' >Next</button>
    </footer>  )
}
